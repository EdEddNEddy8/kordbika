...
if (isset($_GET['opponent'])) {
    ...
    $queryuo = mysqli_query($connect, "SELECT * FROM `players` WHERE username!='$uname' AND character_id>0 AND level>'$mlevel' AND level<'$levelm' AND hp>=10 AND id='$opponent_id' LIMIT 1");
    
} else {
    $queryuo = mysqli_query($connect, "SELECT * FROM `players` WHERE username!='$uname' AND character_id>0 AND level>'$mlevel' AND level<'$levelm' AND hp>=10 ORDER BY rand() LIMIT 1");
}
...
if (isset($_GET['opponent'])) {
    ...
    if ($opponent_id != $player_id && $rowu['energy'] >= 10 && $rowu['hp'] >= 10 && $rowuo['hp'] >= 10 ) {
...
if ($winner_id == $player_id) {
    $zelts = rand(1, 100); // Iegūst gadījuma zeltu
    $respect = 10;
    ...
    $total_experience_gained = $respect + ($total_bonus_xp_percentage * $respect);

    // Update the player's experience points with the total calculated experience
    $player_update = mysqli_query($connect, "UPDATE `players` SET zelts=zelts+'$zelts', energy=energy-10, hp=hp-5, respect=respect+'$total_experience_gained', points=points+5 WHERE id='$player_id'");

    echo '
    <div class="alert alert-success">
      <center><strong><i class="fa fa-trophy"></i> ' . lang_key("you-winf") . ' <br />' . lang_key("reward") . ': <span class="label label-info">' . $total_experience_gained . ' ' . lang_key("respect") . '</span> ' . lang_key("and") . ' <span class="label label-warning">' . $zelts . ' ' . lang_key("zelts") . '</span></strong></center>
    </div>';
}
else {
        ...
        VALUES ('$player_id', '$opponent_id', '$winner_id', '$date', '$time')");
                    $player_update   = mysqli_query($connect, "UPDATE `players` SET points=points-3 , energy=energy-10, health=health-10 WHERE id='$player_id'");
...
if ($rowu['hp'] < 10) {
    echo '
<div class="alert alert-danger">
  <center><strong><i class="fa fa-exclamation-circle"></i> ' . lang_key("no-hptf1") . ' <a href="hospital.php"><i class="fa fa-hospital"></i> ' . lang_key("hospital") . '</a> ' . lang_key("no-hp2") . '</strong></center>
</div>';
}
...
?>
			...
			<center>
			    <div class="col-md-6">
<?php
if ($rowu['energy'] < 10 || $rowu['hp'] < 10 || $countuo <= 0 || $countpac > 0) {
