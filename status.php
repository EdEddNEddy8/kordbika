...
if (isset($_POST['buy-status'])) {
    ...
    if ($rowu['currency'] >= $status_price) {
        ...
        $update_player = mysqli_query($connect, "UPDATE `players` SET currency='$new_currency' WHERE id='$player_id'");
        if ($update_player) {
            // Iegūst esošos statusus no datubāzes
            $current_statuses = $rowu['status'];

            // Pievieno jauno statusu pie esošajiem statusiem, atdalot ar kādu simbolu (piemēram, komatu)
            $new_statuses = $current_statuses . "," . $selected_status;

            // Atjauno statusu kolonnu datubāzē
            $update_status = mysqli_query($connect, "UPDATE `players` SET status='$new_statuses' WHERE id='$player_id'");

            if ($update_status) {
                echo "Jūs esat iegādājies statusu: $selected_status!";
            } else {
                echo "Kļūda, neizdevās atjaunot statusus datubāzē.";
            }
        } else {
